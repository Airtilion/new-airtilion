...
const RecomendationsLoader = () => {
    return (
        <article className="grid [grid-template-columns:350px_298px_40px_298px_350px] [grid-template-rows:298px_298px] gap-[16px] max-2xl:[grid-template-columns:300px_253px_30px_253px_300px] max-2xl:[grid-template-rows:253px_253px] max-xl:[grid-template-columns:300px_328px_300px] max-xl:[grid-template-rows:228px_228px] max-lg:flex lg:flex-wrap max-lg:justify-center">
            {styles?.map((style, index) => (
                    <div key={index} className={`h-full w-full flex flex-col justify-between p-[16px] rounded-[15px] relative overflow-hidden ${style} group max-lg:w-[300px] max-lg:h-[300px] animation-bottom transition-all duration-1000 ease-in-out`} style={{transitionDelay: index*100+'ms'}}>
                        ...
                        <div className='bg-[#EFD8A7] relative z-10 w-fit px-[16px] py-[2px] rounded-[5px] category-span cursor-pointer hover:brightness-75 duration-500 blur-[4px] animate-pulse'>
                            <p className='text-[#242424] text-[14px] max-2xl:text-[12px]'>Lorem ipsum</p>
                        </div>


                        <p className='text-[20px] font-semibold relative z-10 blur-[5px] animate-pulse max-2xl:text-[18px] max-xl:text-[16px]'>Znalazłeś ciekawostkę, napisz do nas</p>

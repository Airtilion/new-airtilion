...
const renderRichText = (content, baseUrl) => {
    if (typeof content === 'string') return content

    return content.map((segment, idx) =>
        typeof segment === 'string'
            ? <React.Fragment key={idx}>{segment}</React.Fragment>
            : <Link key={idx} href={`${baseUrl}${segment.href}`} className='text-white font-bold hover:text-(--hover-text-color) transition-colors'>{segment.text}</Link>
    )
}

const LinkRowListWithTable = ({ dict, baseUrl = "https://airtilion.com" }) => {
    return (
            ...
            <div className='section-style'>
                ...
                <div>
                    <div className='max-w-3xl mx-auto text-center max-md:max-w-none'>
                        ...
                        <p className='mt-6 text-[18px] leading-relaxed max-lg:text-[16px]'>{renderRichText(dict.desc, baseUrl)}</p>
                    </div>
...
export default LinkRowListWithTable
...
const Header = ({full = true, title, content, bg, buttonText}) => {
  return (
    <header className={`mx-24 px-16 py-8 mt-[120px] flex flex-col justify-end gap-32 ${full ? 'h-[calc(100vh-176px)] max-lg:h-[calc(100vh-146px)] max-sm:h-[calc(100vh-102px)]' : ''} relative z-[1] max-2xl:gap-24 max-2xl:mx-16 max-xl:px-12 max-lg:mx-8 max-lg:px-8 max-lg:mt-[90px] max-sm:mx-0 max-sm:px-0 max-sm:pl-6 max-sm:gap-32 max-sm:ml-3`}>
        ...
        <div className='expand flex items-center justify-between bg-[#000000B3] h-[50px] overflow-hidden rounded-full backdrop-blur-[7px] max-2xl:h-[45px] max-2xl:text-[14px] max-xl:text-[13px] max-lg:flex-col max-lg:bg-transparent max-lg:items-start max-lg:gap-4 max-lg:h-auto max-lg:rounded-none max-lg:backdrop-blur-none'>
            ...
            <button className='w-1/2 text-right bg-gradient-to-r from-[#00000000] via-[#E2835040] via-[56%] to-[#E2835040] h-full pr-8 text-white max-2xl:w-1/3 max-xl:w-[60%] max-lg:font-light max-lg:w-full max-lg:text-left max-lg:pr-0 max-lg:h-[45px] rounded-bl-[20px] max-lg:from-[#00000091] max-lg:via-[#E2835091] max-lg:via-[50%] max-lg:to-[#00000040] max-lg:text-[14px] max-lg:px-[32px] max-sm:rounded-bl-[10px] max-sm:from-[#00000040] max-sm:via-[#E2835040]'>{buttonText[1]}</button>
        </div>

        <div className='absolute z-[-1] inset-0 bg-gradient-to-r from-[#00000000] via-[#00000075] max-lg:via-[#000000aa] to-[#000000] to-[89%] rounded-[40px] dark:rounded-none'></div>
        <Image src={bg} width={1736} height={860} quality={100} className='absolute inset-0 z-[-2] rounded-[40px] h-full object-cover opacity-animation max-sm:rounded-[20px]' alt='Zdjęcie tła firmy tworzącej profesjonalne strony internetowe dla firm'/>
    </header>
  )
}

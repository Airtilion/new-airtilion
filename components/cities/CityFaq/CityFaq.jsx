            ...
            <div className='mt-12'>
                {dict.elements.map((q, idx) => (
                    <Accordion open={open === idx} icon={<Icon id={idx} open={open} />} key={idx} className='my-4 transition-all duration-1000 ease-in-out animation-left' style={{transitionDelay: idx*200 + 'ms'}}>
                        <AccordionHeader onClick={() => handleOpen(idx)} className="text-[18px] font-light max-2xl:text-[16px] max-sm:text-[14px] primary-gradient-modified rounded-[5px] pr-6 h-[60px] text-left">{q.q}</AccordionHeader>
                        <AccordionBody className="text-[16px] font-extralight mt-[15px] xxl:text-[20px] xl:text-[15px] lg:text-[13px]">
                            {/* {q.a.map((an, idx) => ( */}
                                <p key={idx} className='text-[16px] max-2xl:text-[15px] max-sm:text-[14px] mt-4'>{q.a}</p>
                            {/* ))} */}
                        </AccordionBody>
                    </Accordion>
                ))}

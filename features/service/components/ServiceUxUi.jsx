            ...
            <div className='section-style '>
                ...
                <div className='relative z-1 grid grid-cols-12 gap-16 max-lg:grid-cols-1'>
                    ...
                    <div className='col-span-6 max-lg:col-span-1'>
                        ...
                        <p className='mt-6 text-[16px] leading-relaxed'>{dict.intro}</p>
                    </div>

                    <div className='col-span-6 max-lg:col-span-1'>

                        <div className='w-full aspect-[21/9] rounded-2xl overflow-hidden relative h-full'>
                            <Image src={uxui} alt="Proces projektowania interfejsu użytkownika (UX/UI)" fill className="object-cover" />
                ...
                <div className='mt-32'>
                    ...
                    <div className='max-w-3xl max-md:max-w-none'>
                        <h3 className='text-[32px]'>{dict.dilemma.title}</h3>
                        <p className='mt-4 text-[16px] leading-relaxed'>{dict.dilemma.desc}</p>
                    </div>

                    <p className='mt-16 text-[28px] font-medium text-white'>{dict.benefitsTitle}</p>

                    <ul className='mt-8 flex gap-4 relative'>

                        {dict.benefits.map((item, idx) => (
                            <li key={idx} className='flex-1 relative overflow-hidden group py-12 px-8 h-[500px]'>
                                <div className='relative z-2'>
                                    <Icon icon={item.icon} width={40} height={40} className='shrink-0 text-white mx-auto group-hover:text-[#E28350] group-hover:scale-110 group-hover:rotate-6 transition-all duration-300' />
                                    <div className='mt-8 text-center'>
                                        <p className='text-[20px] font-medium'>{item.title}</p>
                                        <p className='mt-2 text-[16px] leading-relaxed'>{item.desc}</p>
                                    </div>
                                </div>

                                <div>
                                    <Image src={img[idx]} width={338} height={500} alt='' className='object-cover grayscale-100 brightness-50 absolute inset-0 group-hover:scale-110 duration-300 transition-all' />
                    ...
                    <div className='relative z-1 mt-16 pt-8 border-t border-white/10 flex items-center gap-4'>
                        ...
                        <p className='text-[16px] leading-relaxed'>{dict.conclusion}</p>
                    </div>
                </div>

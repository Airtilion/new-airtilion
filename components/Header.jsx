...
const Header = ({ full = true, title, content, bg, bgMd, buttonText, base = false, base64, dark = false }) => {
  return (
    <header className={`mx-24 px-16 py-8 mt-[120px] flex flex-col justify-end gap-32 ${full ? 'h-[calc(100vh-176px)] max-lg:h-[calc(100vh-146px)] max-sm:h-[calc(100vh-102px)]' : 'h-[570px]'} relative z-[1] max-2xl:gap-24 max-2xl:mx-16 max-xl:px-12 max-lg:mx-8 max-lg:px-8 max-lg:mt-[90px] max-sm:mx-0 max-sm:px-0 max-sm:pl-6 max-sm:gap-32 max-sm:ml-3`}>
      ...
      <picture className='absolute inset-0 z-[-2]'>
        <source srcSet={bgMd.src} media='(max-width: 768px)' />
        <Image src={bg} fill sizes='100vw' quality={100} {...(base && base64 ? { placeholder: 'blur', blurDataURL: base64 } : {})} className={`rounded-[40px] object-cover opacity-animation max-sm:rounded-[20px] ${!full && 'object-top'}`} alt='Zdjęcie tła firmy tworzącej profesjonalne strony internetowe dla firm' />
      </picture>
    </header>
  )
}
